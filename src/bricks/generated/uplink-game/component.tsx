...
export function Component(props: Props) {
  ...
  const [mProg,   setMProg]   = React.useState({arc:0,rec:0,neura:0,aeon:0});
  const [mDone,   setMDone]   = React.useState({arc:false,rec:false,neura:false,aeon:false});
  const [log,     setLog]     = React.useState([
    ...
    '',
    "  Type 'help' for commands or use quick-action buttons.",
    '',
  ]);
  ...
  const traceRef   = React.useRef(trace);   traceRef.current   = trace;
  const credRef    = React.useRef(credits); credRef.current    = credits;
  const hopsRef    = React.useRef(hops);    hopsRef.current    = hops;
  const mProgRef   = React.useRef(mProg);   mProgRef.current   = mProg;
  const mDoneRef   = React.useRef(mDone);   mDoneRef.current   = mDone;
  ...
  React.useEffect(() => {
    ...
    const id = setInterval(() => {
      setTrace(t => {
        const n = parseFloat((t + 0.08).toFixed(2));
        if (n >= 100) { setBusted(true); return 100; }
        return n;
      });
    }, 1000);
  ...
  React.useEffect(() => { bottomRef.current?.scrollIntoView({ behavior:'smooth' }); }, [log]);
  ...
  function run(verb, args) {
    if (verb === 'help') {
      push([
        ...
        '  │  hack <target>  full 4-stage breach                 │',
        '  │  ls             list files on current server        │',
        '  │  trace          show live trace status + ETA        │',
        '  │  reroute        reset proxy (−15–25% trace)         │',
      ...
      push([`  ¢ ${credRef.current.toLocaleString()} — ${credRef.current > 50000 ? 'FLUSH' : credRef.current > 20000 ? 'STABLE' : 'LOW FUNDS'}` ]);
    } else if (verb === 'ls') {
      push([
      ...
      push([
        ...
        tBar(t),
        `  ETA lock: ${Math.floor(eta_s/60)}m ${eta_s%60}s`,
      ...
      push([
        ...
        `  [OK] New exit node: Frankfurt-${Math.floor(Math.random()*9)+1}`,
        '  [OK] TOR bridge relay added',
        `  ✓ Trace −${cut}%. Proxy hops: ${hopsRef.current + 1}`,
      ]);
    } else if (verb === 'bounty') {
      ...
      MISSIONS.forEach(m => {
        ...
        lines.push(`  │  ${m.label.padEnd(22)} ¢${String(m.reward).padStart(6)}  ${st.padEnd(7)} │`);
      });
      lines.push('  └──────────────────────────────────────────────────────┘');
      push(lines);
    } else if (verb === 'connect') {
      const ip = args[0] ?? '0.0.0.0';
      const lat = 150 + Math.floor(Math.random() * 350);
      setTrace(t => Math.min(100, t + 2));
      push([
        `  Routing via ${hopsRef.current}-hop proxy chain…`,
        '  [OK] TOR exit verified',
        `  [OK] Relay authenticated — ${lat}ms latency`,
        `  [OK] Connected to ${ip}`,
        '  Remote OS: UplinkOS 2.1 — ports: 22/ssh 80/http 21/ftp',
        '  ⚠ Trace +2%',
      ]);
    } else if (verb === 'crack') {
      ...
      if (Math.random() > 0.2) {
        ...
        push([
          ...
          '  ✗ Crack failed — ICE countermeasure triggered.',
          '  ⚠⚠ Trace +12%',
        ]);
      }
    } else if (verb === 'hack') {
      ...
      push([
        ...
        '  [1/4] Port scan………………………… ✓',
        '  [2/4] Exploit CVE-2024-9182… ✓',
        '  [3/4] Privilege escalation… ✓ root',
        '  [4/4] Backdoor installed……… ✓',
      ...
      if (cur >= m.steps.length) {
        ...
        setCredits(c => {
          push([`  ✓✓ MISSION COMPLETE: ${m.label}`, `  ¢${m.reward.toLocaleString()} deposited. Balance: ¢${(c+m.reward).toLocaleString()}`]);
          return c + m.reward;
        });
      } else {
        ...
        push([
          ...
          isLast ? `  ✓ Final step! Run 'mission ${id}' once more to collect ¢${m.reward.toLocaleString()}.`
    ...
    <div style={{fontFamily:'"Courier New",monospace',display:'flex',flexDirection:'column',gap:10}}>
      ...
      {busted && (
        <div style={{background:'#1a0000',border:'2px solid #ef4444',borderRadius:8,padding:16,textAlign:'center',boxShadow:'0 0 40px #ef444466'}}>
          ...
          <button onClick={()=>{ setTrace(12); setBusted(false); setHops(4); setCredits(c=>Math.max(0,c-5000)); setLog(['  [!] Emergency reboot initiated.','  [!] ¢5,000 seized as penalty.','  New proxy chain established.','']); }}
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:5}}>
        {MISSIONS.map(m => {
          const prog = mProg[m.id]; const done = mDone[m.id];
          ...
          return (
            <div key={m.id} style={{background:'#06000f',border:`1px solid ${done?'#064e3b':'#3b1a6e'}`,borderRadius:4,padding:'5px 8px'}}>
              ...
              {!done && prog < m.steps.length && (
                <div style={{color:'#4b5563',fontSize:9,marginTop:2,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>↳ {m.steps[prog]}</div>
              )}
            </div>
          );
        })}
      ...
      {/* Terminal */}
      <div style={{background:'#06000f',border:'1px solid #7c3aed',borderRadius:8,padding:'10px 12px',boxShadow:'0 0 24px #7c3aed33'}}>
        <div style={{color:'#6b21a8',fontSize:10,letterSpacing:2,marginBottom:5,borderBottom:'1px solid #2d1060',paddingBottom:4}}>◈ TERMINAL — root@uplink:~$</div>
        <div style={{maxHeight:260,overflowY:'auto',whiteSpace:'pre-wrap',wordBreak:'break-all',fontSize:12,lineHeight:1.55}}>
          {log.map((line, i) => {
            const col =
              ...
              line.startsWith('  [1/') || line.startsWith('  [2/') || line.startsWith('  [3/') || line.startsWith('  [4/') ? '#c084fc' :
        ...
        <div style={{display:'flex',alignItems:'center',marginTop:6,borderTop:'1px solid #2d1060',paddingTop:5,gap:6}}>
          ...
          <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={onKey} disabled={busted}
            ...
            placeholder={busted?'CONNECTION TERMINATED':'type a command…'}
      ...
      <div style={{display:'flex',gap:5,flexWrap:'wrap'}}>
        {['reroute','trace','bounty','ls','whoami','help'].map(cmd => (
        ...
        {MISSIONS.map(m => (
          <button key={m.id} onClick={()=>submit(`mission ${m.id}`)} disabled={mDone[m.id]}
            style={{background:mDone[m.id]?'#022c22':'transparent',border:`1px solid ${mDone[m.id]?'#34d399':'#4c1d95'}`,color:mDone[m.id]?'#34d399':'#a855f7',fontFamily:'inherit',fontSize:10,padding:'4px 10px',borderRadius:4,cursor:mDone[m.id]?'default':'pointer',letterSpacing:.5}}>
